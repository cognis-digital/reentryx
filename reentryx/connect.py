"""Native cognis-connect emit for reentryx — forward findings to any platform.

Maps reentryx's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    reentryx ... --format json | reentryx-emit --to stix
    reentryx-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        ...
        out['description'] = rec.get('location', {}).get('message', '')
