...
<img src="https://capsule-render.vercel.app/api?type=rect&color=0:6b46c1,100:2b6cb0&height=120&section=header&text=REENTRYX&fontSize=48&fontColor=ffffff&fontAlignY=58" width="100%" alt="REENTRYX"/>
...
<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=18&duration=3500&pause=1000&color=6B46C1&center=true&vCenter=true&width=720&lines=Static++symbolic+detector+that+flags+reentrancy+crossfunctio;Self-hostable+%C2%B7+MCP-native+%C2%B7+CI-ready+%C2%B7+polyglot" width="720"/>

[![PyPI](https://img.shields.io/pypi/v/cognis-reentryx.svg?color=6b46c1)](https://pypi.org/project/cognis-reentryx/) [![CI](https://github.com/cognis-digital/reentryx/actions/workflows/ci.yml/badge.svg)](https://github.com/cognis-digital/reentryx/actions) [![License: COCL 1.0](https://img.shields.io/badge/License-COCL%201.0-2b6cb0.svg)](LICENSE) [![Suite](https://img.shields.io/badge/Cognis-Neural%20Suite-6b46c1.svg)](https://github.com/cognis-digital)
...
Read-only reentrancy is the 2023-2025 exploit class most linters still miss; a focused, fast, MCP-native detector that PRs can block on goes viral with audit firms.

`reentryx` is single-purpose, scriptable, and self-hostable: point it at a target, get prioritized results in the format your workflow already speaks (table · JSON · SARIF), gate CI on it, and let agents drive it over MCP.
...
pip install cognis-reentryx
reentryx --version
reentryx scan .                       # scan current project
reentryx scan . --format json         # machine-readable
reentryx scan . --fail-on high        # CI gate (non-zero exit)
```
...
```text
$ reentryx scan .
  [HIGH    ] REE-001  example finding             (./src/app.py)
  [MEDIUM  ] REE-002  another signal              (./config.yaml)
...
```mermaid
flowchart LR
  IN[input] --> P[reentryx<br/>analyze + score]
  P --> OUT[report]
```
...
`reentryx` is interoperable with every popular way of using AI:

- **MCP server** — `reentryx mcp` (Claude Desktop, Cursor, Cognis.Studio, [uncensored-fleet](https://github.com/cognis-digital/uncensored-fleet))
- **OpenAI-compatible / JSON** — pipe `reentryx scan . --format json` into any agent or LLM
...
*Built in the spirit of **Slither**, re-framed the Cognis way. Missing a credit? Open a PR.*
...
Pipes into your stack: **SARIF** for code-scanning, **JSON** for anything, an **MCP server** (`reentryx mcp`) for AI agents, and a webhook forwarder for SIEM/Slack/Jira. See [`docs/INTEGRATIONS.md`](docs/INTEGRATIONS.md).
...
```bash
pip install "git+https://github.com/cognis-digital/reentryx.git"    # pip (works today)
pipx install "git+https://github.com/cognis-digital/reentryx.git"   # isolated CLI
uv tool install "git+https://github.com/cognis-digital/reentryx.git" # uv
pip install cognis-reentryx                                          # PyPI (when published)
docker run --rm ghcr.io/cognis-digital/reentryx:latest --help        # Docker
brew install cognis-digital/tap/reentryx                             # Homebrew tap
curl -fsSL https://raw.githubusercontent.com/cognis-digital/reentryx/main/install.sh | sh
```

| Linux | macOS | Windows | Docker | Cloud |
|---|---|---|---|---|
| `scripts/setup-linux.sh` | `scripts/setup-macos.sh` | `scripts/setup-windows.ps1` | `docker run ghcr.io/cognis-digital/reentryx` | [DEPLOY.md](docs/DEPLOY.md) (AWS/Azure/GCP/k8s) |
...
> ### ⭐ If `reentryx` saved you time, **star it** — it genuinely helps others find it.
