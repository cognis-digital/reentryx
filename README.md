...
[![PyPI](https://img.shields.io/pypi/v/cognis-reentryx.svg)](https://pypi.org/project/cognis-reentryx/)
[![CI](https://github.com/cognis-digital/reentryx/actions/workflows/ci.yml/badge.svg)](https://github.com/cognis-digital/reentryx/actions)
...
REENTRYX exists for one job — static + symbolic detector that flags reentrancy, cross-function, and read-only reentrancy paths in solidity/vyper with ci-gating sarif output. — and does it without a SaaS bill or heavyweight setup.
...
```bash
reentryx --version
reentryx scan .                      # scan the current project
reentryx scan . --format json
reentryx scan . --fail-on high       # non-zero exit for CI gates
reentryx mcp                         # expose as an MCP server (Cognis.Studio / Claude Desktop / Cursor)
```
...
Built in the spirit of **Slither**, re-framed for the Cognis approach: single-purpose, self-hostable,
MCP-native, and unified with the rest of the Suite. Missing a credit? Open a PR.

## How it fits the Cognis Neural Suite

`reentryx` is one of the **100+ tools** in the [Cognis Neural Suite](https://github.com/cognis-digital).
